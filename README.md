...
얼굴 인식 모델의 top-1 결과를 출석 처리에 바로 사용하면 흐린 등록 사진, 비슷한 후보와 중복 검출 때문에 잘못된 기록이 남을 수 있습니다. 인식 결과를 사용자, IN/OUT 상태와 로그에 연결하는 업무 흐름도 필요합니다.

이 프로젝트는 RetinaFace와 ArcFace를 multi-frame enrollment, ambiguity rejection, FastAPI·MariaDB·React 출결 흐름으로 연결한 설계입니다.
