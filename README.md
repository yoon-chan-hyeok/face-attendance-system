...
**기존 얼굴 출결 시스템에서 ambiguity failure를 재현하고, enrollment·matching·attendance decision을 다시 설계했습니다.**
...
[Problem](#1-upgrade-context) · [Before → After](#2-before--after) · [Design](#3-design-decisions) · [Pipelines](#4-end-to-end-pipelines) · [Implementation](#5-current-implementation) · [Quick start](#6-quick-start) · [Validation](#8-validation-status-and-deployment-boundary)
...
조명, 명암과 촬영 위치를 바꿔 테스트하던 중 **닮은 사람이 잘못 승인되는 ambiguity failure**를 확인했습니다. 당시 판정은 가장 가까운 후보가 threshold만 통과하면 승인하는 방식이라, 1순위와 2순위 후보가 거의 비슷한 경우도 둘 중 한 명을 선택할 수 있었습니다.

이 failure를 단순히 threshold 조정 문제로 보지 않고 다음 네 지점을 나눠 확인했습니다.

- **Enrollment** — 사진 한 장이 사용자의 얼굴 변화를 충분히 대표하는가?
- **Candidate retrieval** — 등록 인원이 늘어날 때 모든 샘플을 처음부터 비교해야 하는가?
- **Final decision** — 가장 가까운 후보가 있다는 사실만으로 승인해도 되는가?
...
| 후보 검색 | 전체 사용자와 단순 1:N 비교 | centroid Top-k 후보 축소 → sample 재비교 | 모든 sample을 처음부터 비교하는 비용을 줄이면서 실제 sample을 최종 근거로 유지 |
...
| 다중 얼굴 | 단일 얼굴 중심 | multi-face + 사용자 단위 deduplication | 같은 사용자가 중복 매핑되어 IN/OUT이 여러 번 토글되는 문제 방지 |
| 운영 가시성 | 제한적 | 실시간 로그, 실패 사유, 사용자 조회·삭제 UI | threshold/margin/no-face 등 실패 원인을 빠르게 확인하기 위해 |
...
즉 centroid 하나로 사용자를 완전히 대표하려는 것이 아니라, **centroid는 검색용, sample은 최종 검증용**으로 역할을 분리했습니다.
...
이 구조에서 **centroid는 속도와 안정성을 위한 1차 filter**, **sample reranking은 정확도와 신뢰성을 위한 최종 verification**입니다.
